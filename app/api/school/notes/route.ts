...
import { getSchoolSnapshot, saveNotes } from "@/lib/server/school-store"
import { requireSchoolReadAccess, requireSchoolWriteAccess } from "@/lib/server/school-route"
import type { SchoolNote } from "@/lib/school-data"
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ notes: snapshot.notes })
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    const body = (await request.json()) as { notes?: SchoolNote[] }
    const notes = await saveNotes(Array.isArray(body.notes) ? body.notes : [])
